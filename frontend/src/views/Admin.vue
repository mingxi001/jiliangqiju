  ...
  <div class="admin-container">
    <el-card>
      <div slot="header" class="card-header">
        <span>管理员模块</span>
        <el-button v-if="adminAuthenticated" type="primary" size="small" style="float: right; margin-right: 10px;" @click="changePassword">修改密码</el-button>
      </div>
      ...
      <div v-else class="admin-functions">
        <div class="button-group">
          ...
          <el-button type="primary" @click="openUserManagementDialog">用户管理</el-button>
        </div>
    ...
    <el-dialog title="编辑计量柜" v-model="showEditCabinetDialog" width="30%">
      <el-form ref="editCabinetFormRef" :model="editCabinetForm" :rules="editCabinetRules" label-width="100px">
        <el-form-item label="选择计量柜" prop="id">
          <el-select v-model="editCabinetForm.id" placeholder="请选择计量柜" @change="onCabinetSelect">
        ...
        <el-form-item label="计量柜地址" prop="location">
          <el-input v-model="editCabinetForm.location" placeholder="请输入计量柜地址"></el-input>
        </el-form-item>
        <el-form-item label="计量柜状态" prop="status">
    ...
    <!-- 用户管理对话框 -->
    <el-dialog title="用户管理" v-model="showUserManagementDialog" width="80%">
      <el-table :data="users" style="width: 100%">
        <el-table-column prop="id" label="用户ID" width="80"></el-table-column>
        <el-table-column prop="name" label="用户名" width="180"></el-table-column>
        <el-table-column prop="lastLoginTime" label="最后登录时间"></el-table-column>
        <el-table-column prop="permissions" label="权限设置" width="300">
          <template #default="scope">
            <div v-for="permission in permissions" :key="permission.id" class="permission-item">
              <span>{{ permission.description }}:</span>
              <el-switch
                v-model="scope.row.permissions[permission.name]"
                @change="updateUserPermission(scope.row.id, permission.name, $event)"
              ></el-switch>
            </div>
          </template>
        </el-table-column>
      ...
      <template #footer>
        <el-button @click="showUserManagementDialog = false">关闭</el-button>
      </template>
    </el-dialog>
  </div>
</template>
...
export default {
  ...
  setup() {
    ...
    const handleAdminAuth = () => {
      ...
      // 检查是否启用了登录功能
      const loginEnabled = localStorage.getItem('permission_canEnableLogin') !== 'false'
      if (!loginEnabled) {
        ElMessage.error('登录功能已被禁用')
        return
      }
    ...
    const permissions = ref([
      ...
      { id: 6, name: 'canEnableLogin', description: '启用登录功能', enabled: true }
    ])

    // 更新权限状态
    const updatePermission = (permission) => {
      ElMessage.success(`已${permission.enabled ? '启用' : '禁用'}权限: ${permission.description}`)
      // 保存权限状态到localStorage
      localStorage.setItem(`permission_${permission.name}`, permission.enabled)
    
      // 特殊处理登录功能权限
      if (permission.name === 'canEnableLogin' && !permission.enabled) {
        // 如果禁用了登录功能，确保管理员已登出
    ...
    const onCabinetSelect = () => {
      if (editCabinetForm.value.id) {
        const selectedCabinet = store.state.cabinets.find(cabinet => cabinet.id === parseInt(editCabinetForm.value.id))
        if (selectedCabinet) {
          editCabinetForm.value.name = selectedCabinet.name
          editCabinetForm.value.location = selectedCabinet.location || ''
          editCabinetForm.value.status = selectedCabinet.status || 'enabled'
        }
      }
    }
    const editCabinet = () => {
      ...
      const updatedCabinets = store.state.cabinets.map(cabinet => {
        if (cabinet.id === parseInt(editCabinetForm.value.id)) {
          return {
            ...cabinet,
            name: editCabinetForm.value.name,
            location: editCabinetForm.value.location,
      ...
      // 使用mutation更新密钥数据
      store.commit('updateKeyCabinetName', {
        cabinetId: parseInt(editCabinetForm.value.id),
        cabinetName: editCabinetForm.value.name
      })
      
      ElMessage.success('计量柜编辑成功，相关数据已同步更新')
    ...
    const filteredRecords = computed(() => {
      ...
      return records.value.filter(record => {
        if (searchType.value === 'all') {
          return (
            (record.instrumentName && record.instrumentName.includes(searchQuery.value)) ||
            (record.employeeName && record.employeeName.includes(searchQuery.value)) ||
            (record.status && record.status.includes(searchQuery.value))
          );
        } else if (searchType.value === 'instrumentName') {
          return record.instrumentName && record.instrumentName.includes(searchQuery.value);
        } else if (searchType.value === 'userName') {
          return record.employeeName && record.employeeName.includes(searchQuery.value);
        } else if (searchType.value === 'status') {
          return record.status && record.status.includes(searchQuery.value);
        }
        return true;
      });
    })
...
const users = computed(() => {
  const userData = store.getters.allUsers || []
  console.log('用户数据:', userData)
  // 确保我们有用户数据
  if (userData.length === 0) {
    console.error('没有获取到用户数据，请检查store中的allUsers getter')
  }
  return userData
})

// 打开用户管理对话框
const openUserManagementDialog = () => {
  console.log('打开用户管理对话框')
  showUserManagementDialog.value = true
}

// 更新用户权限
const updateUserPermission = (userId, permissionName, value) => {
store.commit('updateUserPermission', {
userId,
permissionName,
value
})
